Public Class frm_actualizar_familia
    ...
    Private Sub frm_actualizar_familia_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        txt_familia.Focus()
    End Sub

    Public Sub obtener_familia(codigo As String)
        Dim dt As DataTable
        dt = familia.listar("SELECT identificador, descripcion, nivel, codigo, ultimo_nivel FROM familias WHERE codigo='" + codigo + "'")
    ...
    Private Sub actualizar()
        ...
        If familia.actualizar(ULTIMO_NIVEL, IDENTIFICADOR, txt_familia.Text.ToString) Then
            util.mensajes("Familia Actualizada", TITULO_MSJ, "INF")
            frm_familias.listar_familias()
            Close()
        Else
            util.mensajes("Familia no Actualizada", TITULO_MSJ, "WAR")
        End If
