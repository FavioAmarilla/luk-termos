Public Class frm_vista_familia_materia_prima_all
    Dim util As New utilidades
    Private familias As New func_familias_materia_prima
    Dim dt_familias As New DataTable
    Public FRM_ACTIVO As String = ""

    Private Sub frm_vista_familia_materia_prima_all_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        listar_familias()
    End Sub


    Public Sub listar_familias()
        dt_familias = familias.listar("SELECT descripcion, identificador, codigo, nivel, ultimo_nivel, id_linea_padre FROM familias_materia_prima")
    ...
    Private Function obtener_identificador(codigo As String) As Integer
        ...
        Dim id As Integer
        dt = familias.listar("select identificador from familias_materia_prima where codigo = '" & codigo & "'")
    ...
    Private Sub enviar_codigo(ByVal codigo As String)
        If FRM_ACTIVO = "FrmExistenciaStockItems" Then
            FrmExistenciaStockItems.obtener_familia_material(obtener_identificador(codigo), codigo)
            Close()
        End If
        If FRM_ACTIVO = "frm_stock_valorizado" Then
            frm_stock_valorizado.obtener_familia_material(obtener_identificador(codigo))
            Close()
        End If
    End Sub
