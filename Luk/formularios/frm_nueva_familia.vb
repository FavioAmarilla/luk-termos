Public Class frm_nueva_familia
    ...
    Public Sub obtener_familia_padre()
        ...
        dt = familia.listar("select descripcion from familias where identificador=" & IDENTIFICADOR)
    ...
    Private Function obtener_nivel() As Integer
        Dim dt As New DataTable
        dt = familia.listar("select coalesce(nivel, 1) + 1 from familias where identificador=" & IDENTIFICADOR)
    ...
    Private Function obtener_codigo_padre() As String
        ...
        dt = familia.listar("select codigo from familias where COALESCE(id_linea_padre, 0) <= 0 order by identificador desc limit 1")
    ...
    Private Function obtener_codigo_sub(nro_item As Integer) As String
        ...
        dt = familia.listar("select codigo from familias where identificador=" & IDENTIFICADOR)
        If dt.Rows.Count > 0 Then
    ...
    Private Function obtener_nro_item() As Integer
        ...
        dt = familia.listar("select nro_item from familias where id_linea_padre=" & IDENTIFICADOR & " order by nro_item desc limit 1")
    ...
    Private Sub frm_nueva_familia_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        obtener_datos()
    End Sub
